...
Sources: RSS feeds + RBI + 4 News APIs
"""
...
def is_global_announcement(text):
    t = text.lower()
    return any(kw in t for kw in GLOBAL_KEYWORDS)

def fetch_rbi_releases():
    articles = []
    try:
        ...
        for e in entries[:20]:
            ...
            pub   = parse_date(e)
            articles.append({
                'symbol': '','title': f"[RBI] {title}",'url': link,
                'source': 'RBI India','tag_source_name': 'Reserve Bank of India (Official)',
                'published_at': pub,'full_text': clean_html(e.get('summary','')),
                'tag_feed': 'global','tag_category': 'official',
                'agent_source': 'E','tag_after_hours': is_after_hours(pub),
            })
...
def run() -> int:
    ...
    # ── 4 News APIs ───────────────────────────────────────────────────────────
    api_arts = fetch_all_apis("RBI Federal Reserve interest rate India economy global policy", agent_source="E")
    new_api  = [a for a in api_arts if a["url"] not in seen_urls]
    for a in new_api:
        seen_urls.add(a["url"])
        a["tag_feed"]     = "global"
        a["tag_category"] = "official"
    articles += new_api

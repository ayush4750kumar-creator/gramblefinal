...
function Spinner({ size=20 }) {
  return (
    <>
      <div style={{width:size,height:size,border:'2px solid #e5e7eb',borderTop:`2px solid ${UP}`,borderRadius:'50%',animation:'_sa_spin .8s linear infinite',flexShrink:0}}/>
...
function AllNewsTab({ symbol }) {
  ...
  const SENT_STYLE = {
    bullish:  { color: UP, label: '▲ BULLISH' },
    positive: { color: UP, label: '▲ BULLISH' },
    bearish:  { color: DN, label: '▼ BEARISH' },
    negative: { color: DN, label: '▼ BEARISH' },
  };
        ...
        return(
          ...
          >
            ...
            <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:6,flexWrap:'wrap'}}>
              {sent && (
                <>
                  <span style={{fontSize:11,fontWeight:700,color:sent.color}}>{sent.label}</span>

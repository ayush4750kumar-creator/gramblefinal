...
export default function LeftSidebar({ user, token, watchlist, setView, view, onWatchlist }) {
  ...
  return (
    <aside style={{
      ...
      {/* User card */}
      <div style={{
        background: '#1e2433', borderRadius: 10,
        padding: '16px 14px', display: 'flex',
        alignItems: 'center', gap: 12,
      }}>
        <div style={{
          width: 42, height: 42, background: '#2563eb', borderRadius: 9,
        ...
        <div>
          <div style={{ fontWeight: 600, fontSize: 15, color: '#fff' }}>
            {user ? (user.display_name || user.email) : 'User'}
          </div>
      ...
      <div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#9ca3af', textTransform:'uppercase', letterSpacing:'1px' }}>
            Watchlist
          </div>
          {watchlist.length > 0 && (
            <button
              onClick={() => setEditMode(!editMode)}
              style={{ background:'none', border:'none', fontSize:11, color: editMode ? '#dc2626' : '#9ca3af', cursor:'pointer', fontWeight:600, padding:0 }}
            >
              ...
              return (
                <div key={w.symbol} style={{
                  ...
                  <span
                    ...
                    style={{ fontWeight:700, fontSize:14, color:'#2563eb', flex:1 }}
                  >{w.symbol}</span>
                  {editMode && (
                    <span
                      onClick={() => handleRemove(w.symbol)}
                      style={{ fontSize:16, color:'#dc2626', cursor:'pointer', fontWeight:700, padding:'0 4px', lineHeight:1 }}
                    >×</span>
                  )}
                </div>
              );
